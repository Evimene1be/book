...
def donation():
    ...
    if request.method=="GET":
        ...
        don=Donation( don_userid=userid, don_amt=amount,don_fullname=fullname, don_email=email, don_refno=refno, don_status="pending")
        db.session.add(don)
        db.session.commit()
        session["ref"]=refno
...
def make_payment():
    ...
    if session.get('ref')!=None:
        ref = session['ref']
        trxdeets=db.session.query(Donation).filter(Donation.don_refno).first()
...
def paystack():
    if session.get('ref')!=None:
        ...
        headers={"Content-Type":  "application/json", "Authorization":"Bearer sk_test_ad60bdb47ad4985abc033f5217efe6ee6746190c"}
